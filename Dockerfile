...
LABEL maintainer="Ryan Smith (@rms1000watt) FORKED FROM DJ Enriquez (@djenriquez)"
...
RUN set -x && \
    ...
    rm -rf /tmp/glibc-${GLIBC_VERSION}.apk /var/cache/apk/* || true && \
    ...
    rm -rf "$GNUPGHOME" /usr/local/bin/gosu.asc || true && \
...
RUN set -x \
  ...
  && chmod +x /bin/nomad \
  && rm -rf "$GNUPGHOME" nomad_${NOMAD_VERSION}_linux_amd64.zip nomad_${NOMAD_VERSION}_SHA256SUMS nomad_${NOMAD_VERSION}_SHA256SUMS.sig || true \
  && apk del .nomad-deps

RUN set -x && \
    apk --update add --no-cache ca-certificates gettext
